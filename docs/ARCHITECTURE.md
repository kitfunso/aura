...
- `tag.js` owns the session tag: the session key an agent's environment implies, and the read/write of `state.tags`. It emits no escapes and spawns no adapter, so nothing in it can stall a prompt.
