...
## Core Features (MVP)
1. **Repo color identity.** Deterministic mapping: repo identity -> hue. Same repo always gets the same color, in every window, across restarts. Branch shades only exist inside repos. Outside a repo the identity comes from a tag or the tab name (features 8 and 9), and a session with none of the three gets no color at all.
...
9. **A third identity source, for a session nobody will tag.** The tab's own name is read through the frame adapter and used as the identity when there is no repo and no tag. Deterministic like a repo: the tab named `intraday` is the same color on every machine and every restart. Only a title that behaves like a name is taken - under 40 characters, no `·` of aura's own, no path, not a shell default, and the same string on two different prompts, which is what separates a name from an agent's status line. A session colored this way writes no title of its own. Added 2026-08-31 after `state.tags` held exactly ONE entry a day into use: a fix that needs a command per session is a fix nobody runs.
...
8. **NOT an identity for windows with no project.** A session with no repo, no tag and no tab name gets no color at all: no tint, no tab color, no frame. The window keeps the terminal's own default, which is what "no project" should look like. A hue-cycling frame shipped for this case on 2026-08-30 and was removed the same day: coloring a window that names nothing made every window look assigned. A renamed tab is not that case; the name IS the project.
