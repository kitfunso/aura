...
5. **Hook runtime budget: 50 ms.** No network calls, no npm runtime dependencies, no spawning PowerShell on the per-prompt path (frame paints reuse the cached HWND). Why: aura must never make a Claude Code turn feel slower.
...
- Identifying the window by PID ancestry - Windows Terminal is often one process for many windows; use the nonce-title handshake.
- Assuming the title we set persists - Claude Code may rewrite the terminal title; title is re-asserted per prompt and is best-effort.
