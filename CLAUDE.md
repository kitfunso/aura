...
- Trusting a window title that merely LOOKS like a name - an un-renamed tab reports whatever the agent inside wrote, and Claude Code's title (`✳ <latest prompt>`, measured 2026-08-31) passes every static guard. A name is the one that holds still: `settleWindowName` needs two prompts to read the same string before the title becomes an identity.
