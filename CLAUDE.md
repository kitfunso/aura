...
5. **Hook runtime budget: keep the steady-state path at ~70 ms (measured median on this box; node startup ~60 ms dominates, so 50 ms is not reachable with a node hook).** Concretely: no network calls, no npm runtime dependencies, at most ONE git spawn, and no PowerShell spawn on the steady path (the DWM color persists and `vtHex` caches VT delivery; the adapter spawns only on first paint, color change, or undelivered VT - that one-time spawn with VT delivery costs ~1 s, measured). Why: aura must never make a Claude Code turn feel slower.
